...
const skills = [
  { name: "Python", icon: Code2, category: "lang" },
  { name: "LangChain", icon: Brain, category: "ai" },
  { name: "LangFlow", icon: Bot, category: "ai" },
  { name: "LlamaIndex", icon: Brain, category: "ai" },
  { name: "Pydantic", icon: Code2, category: "lang" },
  { name: "Crew AI", icon: Bot, category: "ai" },
  { name: "n8n", icon: Server, category: "infra" },
  { name: "Google AI (ADK)", icon: Cloud, category: "ai" },
  { name: "spaCy", icon: Brain, category: "ai" },
  { name: "FastAPI", icon: Terminal, category: "lang" },
  { name: "PostgreSQL", icon: Database, category: "infra" },
  { name: "Docker", icon: Server, category: "infra" },
  { name: "Git", icon: GitPullRequest, category: "infra" },
  { name: "Linux", icon: Terminal, category: "infra" },
  { name: "Haskell", icon: Code2, category: "lang" },
  { name: "C", icon: Code2, category: "lang" },
  { name: "Azure", icon: Cloud, category: "infra" },
  { name: "Selenium", icon: Bot, category: "ai" },
];
...
const githubProjects = [
  {
    name: "TARS",
    description: "AI assistant project built with Python for intelligent task automation and conversation.",
    language: "Python",
    stars: 1,
    link: "https://github.com/raphaelchristi/tars",
  },
  {
    name: "Claude Master",
    description: "AI-powered development workflow tool for streamlining coding tasks with Claude.",
    language: "Shell",
    stars: 3,
    link: "https://github.com/raphaelchristi/claude-master",
  },
  {
    name: "LLM Studies",
    description: "Jupyter notebooks exploring LLM frameworks, generative AI patterns, and model fine-tuning.",
    language: "Jupyter",
    stars: 0,
    link: "https://github.com/raphaelchristi/LLM-Studies",
  },
  {
    name: "Criptografia",
    description: "Number theory and cryptography algorithms implementation for mathematical research.",
    language: "Python",
    stars: 1,
    link: "https://github.com/raphaelchristi/criptografia",
  },
  {
    name: "My Programming Language",
    description: "A custom programming language based on natural language interpretation, built from scratch.",
    language: "Python",
    stars: 0,
    link: "https://github.com/raphaelchristi/my_programming_language",
  },
  {
    name: "Galton-Watson-OPN",
    description: "Mathematical modeling of Galton-Watson branching processes and optimal population networks.",
    language: "TeX",
    stars: 0,
    link: "https://github.com/raphaelchristi/Galton-Watson-OPN",
  },
];
              ...
              <div className="grid md:grid-cols-[280px_1fr] gap-12 items-start">
                ...
                <div>
                  ...
                  <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-6">
                    {[
                      { value: "33+", label: "Public Repos" },
                      { value: "2.1k+", label: "LinkedIn Followers" },
                      { value: "R$50k", label: "Prize Won" },
                      { value: "10k+", label: "IA Devs Participants" },
                    ].map((stat) => (
          ...
          <div className="container mx-auto px-6 max-w-screen-xl">
            ...
            <ScrollReveal delay={0.1}>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-4 flex items-center gap-2">
                <Brain className="h-3.5 w-3.5" /> AI & Machine Learning
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 mb-8">
                {skills.filter(s => s.category === "ai").map((skill) => {
                  const Icon = skill.icon;
                  return (
                    <div key={skill.name} className="group flex items-center gap-3 rounded-lg border border-border/50 bg-card p-3.5 transition-all duration-300 hover:border-primary/30 hover:bg-accent/50">
            ...
            <ScrollReveal delay={0.2}>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-4 flex items-center gap-2">
                <Code2 className="h-3.5 w-3.5" /> Languages & Frameworks
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 mb-8">
                {skills.filter(s => s.category === "lang").map((skill) => {
                  const Icon = skill.icon;
                  return (
                    <div key={skill.name} className="group flex items-center gap-3 rounded-lg border border-border/50 bg-card p-3.5 transition-all duration-300 hover:border-primary/30 hover:bg-accent/50">
                      <Icon className="h-4 w-4 text-muted-foreground transition-colors group-hover:text-primary" />
            ...
            <ScrollReveal delay={0.3}>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-4 flex items-center gap-2">
                <Server className="h-3.5 w-3.5" /> Infrastructure & DevOps
              </p>
            ...
            <ScrollReveal delay={0.4}>
              <div className="mt-10 rounded-lg border border-dashed border-primary/30 bg-primary/[0.03] p-5">
                <p className="text-xs font-semibold text-primary uppercase tracking-widest mb-3 flex items-center gap-2">
                  <BookOpen className="h-3.5 w-3.5" /> Currently Learning
                </p>
                <div className="flex flex-wrap gap-3">
                  {["Kubernetes", "GitLab CI/CD"].map((tech) => (
                    <span key={tech} className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/5 px-4 py-1.5 text-sm font-medium text-primary">
          ...
          <div className="container mx-auto px-6 max-w-screen-xl">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {featuredProjects.map((project, i) => {
                ...
                return (
                  <ScrollReveal key={project.title} delay={i * 0.1} direction={i % 2 === 0 ? "left" : "right"}>
                    <Card className={`group h-full overflow-hidden border-border/50 bg-card transition-all duration-500 hover:border-border ${project.borderGlow} hover:shadow-xl`}>
            ...
            <ScrollReveal delay={0.2}>
              <div className="mt-16">
                <div className="flex items-center justify-between mb-8">
                  <div>
                    <h3 className="text-xl font-semibold text-foreground">Open Source & Research</h3>
                    <p className="text-sm text-muted-foreground mt-1">More projects on GitHub</p>
                  </div>
                  <Link
                    href="https://github.com/raphaelchristi?tab=repositories"
                    target="_blank"
                    rel="noopener noreferrer"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {githubProjects.map((repo) => (
                    <Link
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="group rounded-lg border border-border/50 bg-card p-5 transition-all duration-300 hover:border-primary/30 hover:shadow-md"
                    >
                      <div className="flex items-start justify-between mb-2">
                        <h4 className="text-sm font-semibold text-foreground group-hover:text-primary transition-colors">{repo.name}</h4>
                        {repo.stars > 0 && (
      ...
      <footer className="border-t border-border/50 py-8">
        <div className="container mx-auto px-6 max-w-screen-xl flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            <Link href="https://github.com/raphaelchristi" target="_blank" rel="noopener noreferrer" className="text-muted-foreground/60 transition-colors hover:text-muted-foreground">
              <Github className="h-4 w-4" />
            </Link>
            <Link href="https://www.linkedin.com/in/raphael-valdetaro/" target="_blank" rel="noopener noreferrer" className="text-muted-foreground/60 transition-colors hover:text-muted-foreground">
              <Linkedin className="h-4 w-4" />
            </Link>
            <Link href="mailto:raphaelvaldetaro@gmail.com" className="text-muted-foreground/60 transition-colors hover:text-muted-foreground">
